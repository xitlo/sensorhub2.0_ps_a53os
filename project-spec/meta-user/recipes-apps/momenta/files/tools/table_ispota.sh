...
IspFirmwareOta_Table(){
        ...
        if [ $IMX = $SET ]; then
                ...
                ##9.upgrade ssm3=table_ssm3.bin
                print_log -e ">>>>>>>>>>>>>>imx390>cam[$1], 9-end.update ssm3=table_ssm3.bin, port[tty$tty_port]<<<<<<<<<<<<<9-end"
                api_cmd -U${tty_port} 0x19 max AD > $API_CMD_LOG
                sleep 1
                BYTE_LENGTH=`wc -c $SSM3_NAME`
                BYTE_LENGTH=`echo $BYTE_LENGTH | awk '{print $1}'`
                BYTE_LENGTH=`printf %x $BYTE_LENGTH`
                if [ ${#BYTE_LENGTH} == 3 ]; then
                        HIGHBYTE=`echo $BYTE_LENGTH | cut -b 2-3`
                ...
                print_log "SSM3_NAME:$SSM3_NAME"
                api_cmd -U${tty_port} 0x90 0 0300000000003601${HIGHBYTE}${LOWBYTE}0000 -i $SSM3_NAME > $API_CMD_LOG
                VAR=`cat $API_CMD_LOG`
                #echo "$VAR"
                CommandStatus=`cat $API_CMD_LOG | grep Done`
                if [ "$CommandStatus" = "" ] ; then
                        print_log -e "\e[1;31m==390==cam[$1],9-end.update ssm3=table_ssm3.bin error 9 \e[0m"
                        return 9
                else
                        print_log -e "\e[1;32m==390==cam[$1],9-end.update ssm3=table_ssm3.bin==390==9end ok \e[0m"
                fi
                ##10.upgrade ssm44=table_ssm44.bin
                print_log -e ">>>>>>>>>>>>>>imx390>cam[$1], 10-end.update ssm44=table_ssm44.bin, port[tty$tty_port]<<<<<<<<<<<<<10-end"
                api_cmd -U${tty_port} 0x19 max AD > $API_CMD_LOG
                sleep 1
                ...
                CommandStatus=`cat $API_CMD_LOG | grep Done`
                if [ "$CommandStatus" = "" ] ; then
                        print_log -e "\e[1;31m==390==cam[$1],10-end.update ssm44=table_ssm44.bin error 10 \e[0m"
                        return 10
                else
                        print_log -e "\e[1;32m==390==cam[$1],10-end.update ssm44=table_ssm44.bin==390==10end ok \e[0m"
                fi
                ##11.upgrade ssm45=table_ssm45.bin
                print_log -e ">>>>>>>>>>>>>>imx390>cam[$1], 11-end.update ssm45=table_ssm45.bin, port[tty$tty_port]<<<<<<<<<<<<<11-end"
                api_cmd -U${tty_port} 0x19 max AD > $API_CMD_LOG
                sleep 1
                ...
                print_log "SSM45_NAME:$SSM45_NAME"
                api_cmd -U${tty_port} 0x90 0 2d00000000003601${HIGHBYTE}${LOWBYTE}0000 -i $SSM45_NAME > $API_CMD_LOG
                VAR=`cat $API_CMD_LOG`
                #echo "$VAR"
                CommandStatus=`cat $API_CMD_LOG | grep Done`
                if [ "$CommandStatus" = "" ] ; then
                        print_log -e "\e[1;31m==390==cam[$1],11-end.update ssm45=table_ssm45.bin error 11 \e[0m"
                        return 11
                else
                        print_log -e "\e[1;32m==390==cam[$1],11-end.update ssm45=table_ssm45.bin==390==11end ok \e[0m"
                fi
...
do
{
        ...
        if [ $flag -ne 0 ]; then
                ...
                if [ $rom_type -eq 0 ]; then
                        ...
                        SSM3_NAME=${ROM_DIR}/rom2/table_ssm3.bin
                        SSM44_NAME=${ROM_DIR}/rom2/table_ssm44.bin
                        SSM45_NAME=${ROM_DIR}/rom2/table_ssm45.bin
                        IMX="IMX390"
