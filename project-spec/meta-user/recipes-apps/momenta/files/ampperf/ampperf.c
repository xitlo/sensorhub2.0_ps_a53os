...
static int gap_us = 1000;
static int gap_max_delay_reset = 10;
...
void *receive(void *pth_arg)
{
    ...
    pstSensor->ucHeadHigh = 0xAA;
    pstSensor->ucHeadLow = 0x55;
    pstSensor->ucType = 0;
    iLen = MAX_RPMSG_BUFF_SIZE;
    ...
    while (loop)
    {
        ...
        else if (ret > 0)
        {
            ...
            pstSensor->uiDataLen++;
            pstSensor->uiTimeSec = (uint32_t)start_time.tv_sec;
            pstSensor->uiTimeNsec = (uint32_t)(start_time.tv_usec * 1000);
...
void *perf_analyse(void *pth_arg)
{
    ...
    while (loop)
    {
        ...
        for (i = 0; i < SENSOR_TYPE_NUM; i++)
        {
            ...
            if (0 < debug_level)
            {
                fprintf(stdout, "UpData[%d] cnt/amp_d/amp_m/freq: %u/%d/%d/%d.%03d, last: %u.%06u->%d.%06ld\n",
                        i,
                        astPerfUp[i].uiCnt,
            ...
            _log_info("UpData[%d] cnt/amp_d/amp_m/freq: %u/%d/%d/%d.%03d, last: %u.%06u->%d.%06ld\n",
                      i,
                      astPerfUp[i].uiCnt,
                      astPerfUp[i].iDelayAmpUs,
                      astPerfUp[i].iDelayAmpMaxUs,
            ...
            if ((++astPerfUp[i].uiCntPerf) % gap_max_delay_reset)
            {
        ...
        for (i = 0; i < SENSOR_TYPE_NUM; i++)
        {
            ...
            if (0 < debug_level)
            {
                fprintf(stdout, "DownData[%d] cnt/amp_d/amp_m/freq: %u/%d/%d/%d.%03d, last: %u.%06u->%d.%06ld\n",
                        i,
                        astPerfDown[i].uiCnt,
            ...
            _log_info("DownData[%d] cnt/amp_d/amp_m/freq: %u/%d/%d/%d.%03d, last: %u.%06u->%d.%06ld\n",
                      i,
                      astPerfDown[i].uiCnt,
                      astPerfDown[i].iDelayAmpUs,
                      astPerfDown[i].iDelayAmpMaxUs,
            ...
            if ((++astPerfDown[i].uiCntPerf) % gap_max_delay_reset)
            {
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "d:hg:")) != -1)
    {
        switch (opt)
        {
        case 'd':
    ...
    while (loop)
    {
        bytes_rcvd = read(eptfd, aucRpmsgRecv + DATA_SENSOR_HEADER_LEN, MAX_RPMSG_BUFF_SIZE);
        if (0 >= bytes_rcvd)
            continue;
        ...
        // if (0 == ((++astPerfUp[pstSensor->ucType].uiCnt) % s_stBram.pstA53Data->usSensorAnalysePerid))
        {
...
#endif
            astPerfUp[pstSensor->ucType].uiCnt++;
            ...
            if ( pstSensor->uiDataLen != astPerfUp[pstSensor->ucType].uiDataCount + 1)
            {
                printf("Up lost %d frame: %d -> %d\n", pstSensor->uiDataLen - astPerfUp[pstSensor->ucType].uiDataCount, astPerfUp[pstSensor->ucType].uiDataCount, pstSensor->uiDataLen);
            }
            astPerfUp[pstSensor->ucType].uiDataCount = pstSensor->uiDataLen;
